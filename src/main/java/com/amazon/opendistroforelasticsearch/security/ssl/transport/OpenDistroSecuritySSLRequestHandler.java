...
implements TransportRequestHandler<T> {
    ...
    public OpenDistroSecuritySSLRequestHandler(String action, TransportRequestHandler<T> actualHandler, 
        ...
        this.flag = true;
        log.debug("bdebjani: Action is " + action);
    }
    ...
    public final void messageReceived(T request, TransportChannel channel, Task task) throws Exception {
        ...
        try {
            ...
            // Netty Channel
            String nettyChannelString = nettyChannel.getNettyChannel().id().asLongText();
            ...
            log.debug("peerCerts.length is "+ peerCerts.length);
            ...
            if (peerCerts != null 
                ...
                final String peerCertsString = Arrays.stream(x509PeerCerts).map(c->c.getSubjectDN().getName()).collect(Collectors.toList()).toString();
                final String localCertsString = Arrays.stream(x509LocalCerts).map(c->c.getSubjectDN().getName()).collect(Collectors.toList()).toString();
                log.debug("bdebjani: Peer certs DN " + peerCertsString);
                log.debug("bdebjani: Local certs DN " + localCertsString);
                log.debug("bdebjani: principal " + principal);
