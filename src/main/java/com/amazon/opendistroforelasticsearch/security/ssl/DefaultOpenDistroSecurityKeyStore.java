...
public class DefaultOpenDistroSecurityKeyStore implements OpenDistroSecurityKeyStore {
    ...
    public DefaultOpenDistroSecurityKeyStore(final Settings settings, final Path configPath) {
        ...
        log.info("bdebjani: TLS Transport Client Provider : {}", sslTransportClientProvider);
        log.info("bdebjani: TLS Transport Server Provider : {}", sslTransportServerProvider);
        log.info("bdebjani: TLS HTTP Provider             : {}", sslHTTPProvider);
    ...
    private void initSSLConfig() {
        ...
        if (transportSSLEnabled) {
            ...
            if (rawKeyStoreFilePath != null) {
                ...
                try {
                    ...
                    log.info("bdebjani: ---- about to create keystore instances --- ");
                    ...
                    log.info("bdebjani: ---- exporting server chain --- ");
                ...
                log.info("bdebjani: ----rawPemCertFilePath is not null ");
        ...
        if (!client && httpSSLEnabled) {
            log.info("bdebjani: client looking for keystore and pem store");
            ...
            log.info("bdebjani: pem Cert file path (for client) is (via ssl)"+ rawPemCertFilePath);
            log.info("bdebjani: pem Key file path (for client) is (via ssl)"+ rawKeystoreFilePath);
            ...
            if (rawKeystoreFilePath != null) {
                ...
                log.info("bdebjani: !client && httpSSLEnabled rawPemCertFilePath != null");
                ...
                log.info("bdebjani: pemCertFilePath {}", pemCertFilePath);
                log.info("bdebjani: pemKey {}", pemKey);
    ...
    public SSLEngine createServerTransportSSLEngine() throws SSLException {
        log.info("bdebjani: created Server Transport SSL Engine");
    ...
    public SSLEngine createClientTransportSSLEngine(final String peerHost, final int peerPort) throws SSLException {
        log.info("bdebjani: created Client Transport SSL Engine");
        if (peerHost != null) {
    ...
    private SslContext buildSSLServerContext(final PrivateKey _key, final X509Certificate[] _cert,
        ...
        log.info("bdebjani: Building SSL Server Context");
    ...
    private SslContext buildSSLClientContext(final PrivateKey _key, final X509Certificate[] _cert,
            final X509Certificate[] _trustedCerts, final Iterable<String> ciphers, final SslProvider sslProvider)
            throws SSLException {
        log.info("bdebjani: building SSL Client Context");
